...
func (cri *ItemRepositoryImpl) Items() ([]entity.Item, error) {
	...
	for rows.Next() {
		Item := entity.Item{}
		err = rows.Scan(&Item.name, &Item.catagory, &Item.subcatagory, &Item.price, &Item.quantity, &Item.id)
		if err != nil {
			return nil, err
		}
...
func (cri *ItemRepositoryImpl) Item(id int) (entity.Item, error) {
	...
	err := row.Scan(&c.name, &c.catagory, &c.subcatagory, &c.price, &c.quantity, &c.id)
	if err != nil {
		return c, err
	}
